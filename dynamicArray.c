...
// --- Heap ---
//internal function for adjust. from worksheet
int _smallerIndexHeap(DynamicArray *heap, int i, int j, compareFunction compare);

int _smallerIndexHeap(DynamicArray *heap, int i, int j, compareFunction compare)
{
    if(compare(dyGet(heap, i), dyGet(heap, j)) <= 0)
        return i;
    else
        return j;
}
...
{
    //assert(dyGet(heap, last)>dyGet(heap, position));
    ...
    if(right < last)
    {
        ...
        if(compare(dyGet(heap, small), dyGet(heap, position)) == -1)
        {
            dySwap(heap, position, small);
            adjustHeap(heap, last, small,compare);
        }
    }//if one child
    else if(left < last)
    {
...
{
    // FIXME: implement
    //assert that it is valid and is not empty
    assert(heap != NULL);
    assert(!dyStackIsEmpty(heap));
    int i;
    int max = heap->size;
    for(i=max/2-1;i>=0;i--)//starts at first non leaf node (max/2-1)
        //recursively calls adjust until in correct heap form.
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    //assert (last); /* make sure we have at least one element */
